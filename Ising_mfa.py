...
class IsingMfaPerceptron(nn.Module):
    """"
    mfaPerceptron
    """        
    def __init__(self, input_size, hidden_size, output_size, J):
        super(IsingMfaPerceptron, self).__init__()
...
for J in tqdm(J_array):
    ...
    model = IsingMfaPerceptron(input_size= input_size, output_size= output_size, hidden_size = 16, J = J)
...
dataset = IsingMfaDataset(J_values= J_values,  output_energy_list= output_energy_list, output_vectors_list = output_vectors_list, labels_list= labels_list, output_state_list= output_states_list)
dataloader = DataLoader(dataset, batch_size=1, shuffle=False)

class IsingMfaDatasetLoader():
